...
- `Cmd+O` の従来動作が壊れていない
- タイプして候補が増減したとき、パネルの上端位置が固定されている（高さ変化で上端が動いて見えないか）。`preferredContentSize` 追従の高さ変化は NSWindow 既定では左下（原点）基準のため、上端が動くようなら高さ変化分の `setFrameOrigin` 補正を入れる（TASK-165(10)）
