...
- [ ] **Step 5: `Cmd+.` とシート・モーダルとの競合を確認する**

`Cmd+.` は macOS で伝統的に「キャンセル」のキー割り当てとして使われる(最終レビューでの指摘)。印刷パネル・アップデート確認ダイアログ・ファイル未検出アラートなど、シート/モーダルが最前面にある状態で `Cmd+.` を押し、不可視ファイル表示が意図せず切り替わらないこと、およびシート側のキャンセル操作(あれば)が引き続き機能することを確認する。
