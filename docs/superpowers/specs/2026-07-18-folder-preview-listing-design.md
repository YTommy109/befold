...
> **これは 2026-07-18 時点の設計スナップショットです。**
...
> **注記 (TASK-310 で改訂)**: この節の「移動後は常に `selection = nil`」は現在の実装では
> **上位フォルダーへ移動した場合以外もう成り立たない**。下位・横へ移動したときは一覧の
> 先頭行(`..` を除く)を選択し、それがファイルならプレビューの中身も切り替える。
> サイドバーのハイライトとプレビューの内容が食い違うのは不具合だという判断による。
> 「移動先で自動的にファイルを開かない」という下記の方針はこの範囲で撤回されている。
> 一覧が空(または `..` のみ)のフォルダーでは従来どおり `selection = nil` で、
> プレビューは移動先ディレクトリの一覧になる。
