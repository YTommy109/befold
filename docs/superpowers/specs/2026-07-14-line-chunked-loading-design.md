...
| 形式 | 経路 | サイズ上限 |
|---|---|---|
| CSV/TSV・コード・プレーンテキスト（行指向） | チャンク読み（常時） | なし |
...
- エンコーディングは**先頭チャンクで一度だけ確定**する
  （BOM → UTF-8 → 既存 `decodeUnicodeText` のヒューリスティック）。
  以降のチャンクには確定したデコーダを固定適用する。
  CP932 は 2 バイト目に `0x0A` が現れないため改行バイトでの分割が安全。
- UTF-16/UTF-32（BOM で検出）はバイト単位の改行分割が壊れるため
  チャンク対象外とし、従来の一括経路（10MB 上限）へフォールバックする。
...
- 23MB CSV: 初回 1,000 行（約 47,000 セル）が瞬時に表示され、
