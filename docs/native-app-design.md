...
<!-- derived-from ./superpowers/specs/2026-05-30-mmdview-design.md -->

mmdview を 2 つのプロダクトに分割する。本ドキュメントはその一方である
**macOS ネイティブアプリ（Swift）** のアーキテクチャ設計書である。
もう一方（Web アプリ）は [web-app-design.md](./web-app-design.md) を参照。

- 現行 mmdview（Python + FastAPI + pywebview）と**同等の機能**を Swift で再実装する
- `.mmd` / `.md` ファイルを監視し、mermaid.js でリアルタイムプレビューする
- 将来フェーズとして **AI 編集機能**（表示中ファイルの AI による書き換え）を追加する
...
mmdview.app (Swift / SwiftUI)
  ...
  └── ViewerWebView    # WKWebView
        ├── 同梱アセット（viewer.html / mermaid.min.js / style.css）
        └── JS ブリッジ
...
- mermaid.js・htmx 相当の資産のうち、**mermaid.js と viewer の HTML/CSS/JS のみ**
  アプリバンドルに同梱する（htmx・SSE 拡張・_hyperscript は不要になる）
...
<!-- derived-from ./superpowers/specs/2026-05-30-mmdview-design.md#ファイル変更検知自動リロード -->
...
<!-- derived-from ./superpowers/specs/2026-06-06-multi-windows-design.md -->
<!-- derived-from ./superpowers/specs/2026-06-04-open-with-ext-design.md -->
...
<!-- derived-from ./superpowers/specs/2026-06-06-zoom-design.md -->

viewer.html・style.css・mermaid 初期化設定は現行実装から移植する。

- **mermaid 初期化**: `startOnLoad: false`、全ダイアグラム種別 `useMaxWidth: false`、
  `theme: 'default'`（現行と同一）
- **`.mmd` / `.md` の扱い**: 現行同様、全文を `<pre class="mermaid">` に渡し
  mermaid.js に処理させる（` ```mermaid ` フェンスの自動検出を含む）
...
<!-- derived-from ./superpowers/specs/2026-06-03-auto-upgrade-design.md -->
...
<!-- derived-from ./web-app-design.md#ai-依頼フロー -->

本フェーズの詳細設計は別ドキュメントで行う。ここでは方式候補と
フェーズ 1 で確保しておく拡張ポイントのみ定める。

### 体験イメージ

ビューアに依頼ペインを追加し、「この図に決済フローを追加して」のような依頼で
表示中の `.mmd` / `.md` ファイルを AI が書き換える。書き換え結果はファイルに
保存され、既存のファイル監視がそのままプレビューを更新する。

### 方式候補の比較

| 観点 | A: Claude API 直接呼び出し | B: ローカル CLI エージェント（Claude Code 等） |
|---|---|---|
| 前提 | ユーザーが API キーを設定 | CLI のインストール・認証済みが前提 |
| 実装 | URLSession で Messages API。単一ファイル書き換えに十分 | Process 起動・出力パース。複数ファイル・ツール使用も可能 |
| 配布 | 自己完結 | 外部依存あり |
| 第一候補 | ◎（フェーズ 2 はこちら） | 将来の拡張オプション |

### 安全策（方式によらず必須）

- AI の書き換え結果は**即時保存せず**、差分プレビュー → ユーザー承認 → 保存の順とする
- 保存前に元ファイルのバックアップを作成する（直前 1 世代でよい）
- 承認後の保存はアプリ自身の書き込みでも FileWatcher が発火するため、
  プレビュー更新経路は既存のまま使える

### フェーズ 1 で確保する拡張ポイント

- `ViewerStore` に「表示内容はファイル由来」前提を置かず、
  content の供給元を差し替え可能にしておく（差分プレビュー表示で必要になる）
- 依頼ペイン分のレイアウト余地を viewer.html ではなく SwiftUI 側に持たせる
...
1. **フェーズ 1**: 現行機能パリティの Swift 版を新リポジトリで実装。
   現行 Python 版はメンテナンスモード（バグ修正のみ）
2. パリティ達成後、Swift 版を v1.0 としてリリースし Python 版を置き換える
   （Sparkle 移行のため、初回は手動での入れ替えインストールになる）
3. **フェーズ 2**: AI 編集機能の詳細設計・実装
...
- mermaid 以外のダイアグラム形式
- エクスポート機能（SVG / PNG）
- 人向けのテキスト編集機能（AI 編集の差分承認 UI は含むが、自由編集エディタは持たない）
- AI 編集機能の詳細設計（フェーズ 2 で別ドキュメント化）
