...
| `PageNumberField` | ページ番号を打ち込む一行フィールド（AppKit の `NSTextField` を `NSViewRepresentable` で包む）。**SwiftUI の `TextField` + `@FocusState` では打ち込めない**——この面は AppKit がホストする `PDFView` の上に重なる SwiftUI で、`isInputFocused = true` を入れても窓の first responder は `ZoomingPDFView` のままだった（実測 / TASK-578.2）。窓へ入った 1 周後に `makeFirstResponder` を呼んで明示的に移し、確定（Enter）と取り消し（Esc）も AppKit の `doCommandBy` で受ける。**閉じた後の戻し先はこの型では決めない**——出す前の first responder へ返す形にしていたが、実測ではそれがサイドバーのファイル一覧で、ジャンプ直後に ↓ を押すと選択が動いて別のファイルが開いた。戻し先は `PDFPageIndicatorModel` が面に決め打つ |
