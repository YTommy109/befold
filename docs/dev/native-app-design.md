...
- **文書内ジャンプ**: 文書順に並んだ目印を前後移動する
  ...
  **バーを開いている間は目印の候補に地色が付き**、次にどこへ飛ぶか分かる。
  装飾は**検索と同じ**で、候補が薄い地色（`--hit-bg`。検索の候補ヒットと共有する
  トークン）、現在位置がアクセントの地色に白文字（`mark.mmd-find-match-current`
  と同じ）。同じ「前後移動する UI」なので目印の読み方も検索と揃えてある
  （TASK-485.22）。差分表示だけは例外で、地色を持つ変更行を潰さないよう
  `.diff-table` スコープで打ち消し、上記の左端の縦帯を使う。
