...
- **文書内ジャンプ**: 文書順に並んだ目印を前後移動する
  ...
  **変更ブロック**は差分表示が対象で、連続する削除行とその直後の追加行のまとまりを
  ...
  変更ブロックの現在位置ハイライトは行ではなく**セル**へ当てる（`border-collapse` の表では
  行への outline が上下辺しか描かれない）。当てる先は**ブロック先頭行のガター**
  （行番号セル。行番号を出していない表では記号セル）だけで、変更行の全セルは囲まない
  （行数の多いブロックで枠だらけになり、差分の地色とも競合するため）。差分表示中は本文が段階読み込み中でも
