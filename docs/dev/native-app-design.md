...
| `ViewerWindowKind` | ビューア窓の種別（`.viewer` / `.slide`）。生成時に決まり以後変わらない（`let` で受ける）。`allowsSidebar` / `hasToolbar` / `joinsTabs` / `isRestorable` / `recordsUsageHistory` の 5 つの述語と `defaultContentSize`（`.viewer` は 1100×850、`.slide` は 1280×720 = 16:9）を持ち、**呼び出し側で `kind == .slide` と書かない**。`.slide` はプレゼン用のスライド窓で、サイドバー無し・ツールバー無し・タブ合流なし・セッション復元の対象外・利用履歴に残さない。`recordsUsageHistory` は「最近使った項目」と「最近使ったリポジトリ」の両方を 1 つの述語で扱い、判定は各履歴ストア（`RecentDocumentsStore.noteOpened(_:kind:)` は `kind` を必須引数で受け、`RecentRepositoryRecorder.recordIfNeeded(for:controller:)` は解決を始める前に弾く）の中に置く（TASK-610） |
