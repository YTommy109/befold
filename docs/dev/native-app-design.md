...
| `BookmarkStore` / `BookmarkLibrary`（BefoldKit） | ブックマークの永続化。値は `BookmarkLibrary`（エントリ = 正規化パス + 任意の別名 + 所属フォルダーの名前列、フォルダー = 名前列 + 展開状態）で、UserDefaults キー `Bookmarks` に JSON で 1 つ。純粋な操作（追加・削除・別名・rename 追随・フォルダーの作成/改名/削除/移動/展開状態）は値型 `BookmarkLibrary` にあり（同じ親の下でフォルダー名は一意、削除は配下を親へ繰り上げる、記録の無い所属はルート扱いで読む）、`BookmarkStore` は「読む → 1 操作 → 書く」の薄い層。デコード済みの値をメモリに持ち、自分の書き込みで更新する——前提は「同一プロセスの writer はこのインスタンスだけ」（`AppStores` が 1 個、CLI は GUI 起動中は転送）。旧キー `BookmarkedPaths`（パス配列）は `init` で一度だけ移行し、移行の有無にかかわらず削除する（TASK-536.1）。GUI と CLI（`befold --bookmark`）が同じ実装を使う |
| `BookmarksMenuController` | Bookmarks メニューの一覧を表示直前に作り直す `NSMenuDelegate`。フォルダーはサブメニューとして再帰的に組み、各階層でフォルダーが先・エントリが後（順序は `BookmarkLibrary.children(of:)` が決める）。表示名は `BookmarkEntry.displayName`（別名 ?? ファイル名）で並べ、右列のパスは別名を付けても親ディレクトリのまま。**`FileReading` を持たない**（存在確認は `MissingBookmarksPruner` に閉じる） |
