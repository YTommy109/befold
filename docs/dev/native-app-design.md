...
- [ファイル種別ごとの表示仕様](./file-type-display.md) — 種別ごとの扱い・変換・表示幅
- [ビューアの操作 UI](./viewer-ui.md) — ズーム・検索・文書内ジャンプ・表示モード・サイドバー
...
| `ViewerWindowSessionSync` | close / rename / ファイル切替 / key イベントを受けての辞書のキー付け替えと、セッション・最近使った項目・ブックマークの追随。「開いた」記録の入口は `noteOpened(_:in:)` の 1 つで、セッションは `isRestorable`、利用履歴は `recordsUsageHistory` で窓の種別から決める。スライド窓はセッションに書かず、「閉じた」判定（`noteClosedIfNoWindowRemains`）でも数に入れない（TASK-612）。`remapController` は rename / 切替のどちらでも最後に `noteOpened(_:in:)` を通す（rename 固有の付け替えを先に済ませてから合流させる。TASK-616）。`ViewerWindowControllerDelegate` 準拠（辞書の書き換えはマネージャの `register` / `detach` を通す。窓を作らない関心なのでマネージャから分離した） |
...
| `ViewerTabGrouping` | タブグループ規則（結合・タブ構成スナップショットの組み立て・Window メニューを選択中タブだけに揃える・Space からはぐれた窓の救出）。新しいタブ（`.newTab`）は**背面で開き、表示も焦点も起点に留める**（Safari の cmd+クリックと同じ。`present(select: false)` が表示後に起点タブへ選択を戻す）。セッション保存/復元と最近使ったリポジトリが同じ解釈を共有する単一の置き場。結合の置き場所は `NewTabPlacement`（同じファイルに置く）を**必須引数**で受け、`.end` はグループ末尾、`.afterSource` は起点タブの直後（同じ起点から続けて開いたときは直前の派生タブの直後で、クリック順に並ぶ。起点の `ViewerWindowController.lastSpawnedTab` が直前の派生タブを weak で覚え、同じグループに残っている間だけ使う）。開いた元が決める値で、サイドバー由来（`ViewerWindowController+FileList`）は `.end`、文書内リンク由来（`ViewerWindowController+References`）は `.afterSource` を `openFileElsewhere` クロージャで渡す（TASK-611。Safari がリンク由来のタブを現在のタブの隣に開くのと同じ考え方） |
...
| `ViewerWindowOpenPolicy` | 窓を開くときの純粋な判定（disposition の解釈・既存ウィンドウの再利用規則・サイドバー初期開閉の解決順）。副作用は持たず、候補も記憶も引数で受ける。再利用の判定は 2 段で、**種別の絞り込み（`acceptsReopen`）が disposition に関係なく先に掛かり**、そのあとに disposition ごとの規則が続く（TASK-613 / TASK-614）。`effectiveDisposition(_:relativeTo:)` は起点の窓の種別を見て `.newTab` を解釈し直し、起点がタブへ合流しない種別（スライド窓）なら `.newWindow` へ倒す。**起点の種別で分岐するのはここだけ**で、`openViewer` は戻り値を同名のローカルで shadow して以降が元の要求へ触れないようにする（TASK-615） |
...
| `MainMenuBuilder` | メインメニューをコードで構築。トップレベルは App / File / Edit / View / Bookmarks / Window / Help の 7 つで、ブックマークは追加・削除のトグルと一覧を 1 つのメニューに集約する（TASK-535）。一覧は `BookmarksMenuController` が表示直前に作り直すが、**固定部（トグルと「ブックマークを編集…」）は組み立て時にも `addBookmarksFixedItems(to:)` で置く**——`MenuShortcutCatalog.snapshot` は `NSApp.mainMenu` へ設定する前のメニュー木から取るため、delegate が作る項目は Help のショートカット一覧に載らない。「ブックマークを編集…」のキーは ⇧⌘D（TASK-620.4）。キーの文字は ⌘D のトグルと同じ `BookmarkShortcut.keyEquivalent` から取る。TASK-536.1 では Help の一覧と紹介サイトのショートカット検証に載る項目が増えるのを避けてキー等価を付けていなかったが、入口がメニューだけでは遠いので方針を改めた |
| `MenuShortcutCatalog` / `HelpShortcutSections` | Help > キーボードショートカット に並べる一覧の組み立て。メニュー由来は `NSMenu` から抽出し、メニューを経由しない操作は `ViewerShortcutCatalog` / `SidebarShortcutCatalog` / `QuickOpenShortcutCatalog` から引く。キー表記の組み立ては `ShortcutKey` に集約し、一覧と実装のずれは各カタログの突合テストで落とす。ビューア内の一覧はスクロールのキーに加え、文書内ジャンプの Enter / ⇧Return と、検索バー・ジャンプバーを閉じる Esc を載せる |
...
| `BookmarkStore` / `BookmarkLibrary`（BefoldKit） | ブックマークの永続化。値は `BookmarkLibrary`（エントリ = 正規化パス + 任意の別名 + 所属フォルダーの名前列、フォルダー = 名前列 + 展開状態）で、UserDefaults キー `Bookmarks` に JSON で 1 つ。純粋な操作（追加・削除・別名・rename 追随・フォルダーの作成/改名/削除/移動/展開状態）は値型 `BookmarkLibrary` にあり（同じ親の下でフォルダー名は一意、削除は配下を親へ繰り上げる、記録の無い所属はルート扱いで読む）、`BookmarkStore` は「読む → 1 操作 → 書く」の薄い層。**フォルダー（ディレクトリ）はブックマークできない**（ウィンドウ側のトグルがフォルダー一覧の表示中は無効なのと同じ仕様）。規則は `BookmarkStore.canBookmark` の 1 つで、パスを外から受け取る入口（Bookmark Editor へのドロップ・`befold --bookmark`）がこれで弾く。CLI は GUI へ転送する前に弾く（転送先はパスを判定しない）。以前の版で追加されたフォルダーのブックマークは消さずに表示し続ける（TASK-621）。`BookmarkEntry.url` は `URL(filePath:directoryHint:)` で作りファイルシステムを見ない（`URL(fileURLWithPath:)` は stat するため、表示・ソート・Quick Open の候補収集のたびにブックマーク先を見に行っていた。TASK-620.1）。デコード済みの値をメモリに持ち、自分の書き込みで更新する——前提は「同一プロセスの writer はこのインスタンスだけ」（`AppStores` が 1 個、CLI は GUI 起動中は転送）。**エントリの配列順が各フォルダー内の並び（手動の並び）**で、追加は末尾、並び替えは `move(_:to:before:)`（兄弟の直前、兄弟が行き先に居なければ末尾）、別名や rename 追随では位置を変えない。手動の並びを知らない版が書いた値は `hasManualOrder` を持たず（nil）、表示名順で見せていたので、`init` の一度きり移行で `orderedManually()`（表示名順へ安定ソートして印を付ける）を保存する。旧キー `BookmarkedPaths`（パス配列）からの変換も同じ移行の中で行い、旧キーは移行の有無にかかわらず削除する（TASK-536.1 / TASK-620.3）。旧版が保存し直すと印が落ち、次の起動で表示名順へ並べ直される。GUI と CLI（`befold --bookmark`）が同じ実装を使う |
| `BookmarksMenuController` | Bookmarks メニューの一覧を表示直前に作り直す `NSMenuDelegate`。フォルダーはサブメニューとして再帰的に組み、各階層でフォルダーが先（名前順）・エントリが後（保存順 = Bookmark Editor での手動の並び。順序は `BookmarkLibrary.children(of:)` が決める）。表示名は `BookmarkEntry.displayName`（別名 ?? ファイル名）で並べ、右列のパスは別名を付けても親ディレクトリのまま。項目のアイコンは `BookmarkEntry.iconType`（拡張子だけから決まる `UTType`）の型アイコンで、パスからは引かない（`addFileItems` の `icons` は既定値の無い必須引数で、パスから引く Recent と呼び出し元ごとに選ぶ）。**`FileReading` を持たない**（存在確認は `MissingBookmarksPruner` に閉じる） |
| `HostedPanelPresenter` / `HostedPanelWindowController` | 単一インスタンスのパネル（About / 設定 / ブックマーク管理 / Help 配下）の生成と開閉トグル。`HostedPanel` の case ごとの中身・タイトル・サイズは `makeController` の 1 箇所に置く。パネルからファイルを開く経路は `openHandler`（`DocumentOpener.openViewer`）を既定値なしで受ける |
| `BookmarkManagerModel` / `BookmarkManagerView` | Bookmark Editor（Bookmarks > ブックマークを編集…）。モデルは `BookmarkStore` のスナップショット（`BookmarkLibrary`）を持ち、各操作はストアへ書いてから取り直す。ブックマークの**集合**が変わったとき（削除）だけ `onChange` → `refreshAllToolbars` で全窓へ伝え、別名・フォルダーの変更では呼ばない。Bookmark Editor が key になったときも取り直す（窓側の ⌘D で変わった分を拾う）。一覧はフォルダーの `DisclosureGroup` を再帰で組むツリーで、行の識別は `BookmarkRow`（フォルダー = 名前列 / ブックマーク = 正規化パス）。ブックマーク行は表示名の下に `BookmarkEntry.detailPath` を添える（別名が無ければ親ディレクトリ、別名があればファイル名まで含むパス。別名で表示名からファイル名が消えても何のファイルか分かるように。TASK-620.5）。別名の操作は「別名を設定…」と表示し、ファイル自体の改名と取り違えないようにする。行の操作（ブックマーク行: 別名・フォルダーへ移動・削除 / フォルダー行: 改名・新規フォルダー・削除）は右クリックと行の右端の ⋯（borderless の `Menu`）の 2 つの入口から開け、中身はどちらも同じ `@ViewBuilder`（`bookmarkActions` / `folderActions`）から作る（入口ごとに項目を書くと片方だけ項目が増える形で割れるため。TASK-620.2）。ダブルクリックで開く範囲は ⋯ を除いた行の本体だけ。行のアイコンはサイドバーと同じ種別のアイコンだが、サイドバーの `NSWorkspace.icon(forFile:)` ではなく `BookmarkEntry.iconType` から引く（拡張子の無い名前は汎用の書類アイコン。TASK-620.1）。**stat はしない**（欠落バッジを出さない）。文字入力（別名・フォルダー名）は 1 つの `.alert` + `TextField` を `BookmarkManagerPrompt` で使い回す（インライン編集は持たない）。フォルダーの削除は配下を親へ繰り上げるので確認を挟まず、名前の重複だけを alert で伝える。**Finder からの D&D** はアプリ内で唯一の `onDrop` 実装（`BookmarkManagerView+Drop`）で、Bookmark Editor 全体（→ トップレベル）とフォルダー行（→ そのフォルダー）が `.fileURL` を受ける。受け入れ規則は `BookmarkManagerModel.dropDecision`（純粋関数）：登録済みは数えない / 存在しなければ弾く / フォルダーは弾く（`BookmarkStore.canBookmark`）/ 通常ファイルで対応形式でなければ弾く。存在確認はドロップの瞬間だけ `withBlockingWork` で行い、表示では stat しない約束を保つ。弾いた分はモーダルにせず Bookmark Editor の下部の 1 行で伝える。**Bookmark Editor 内の行 D&D**（TASK-620.3）はブックマーク行だけがドラッグ元（⋯ を除く行の本体に `.onDrag`）で、運ぶ型はアプリ内専用の `com.degino.befold.bookmark-entry`（`UTType.bookmarkEntry`。Info.plist の `UTExportedTypeDeclarations` に宣言しないと挿入線は出てもドロップが届かない。実測）。`.fileURL` は載せないので Finder からの追加の受け口と型で分かれる。行間へのドロップは各段のエントリの `ForEach` の `.onInsert` → その段の兄弟の直前へ、フォルダー行と Bookmark Editor 全体の `.onDrop` は内部型ならそのフォルダー（トップレベル）の末尾へ移し、`.fileURL` なら従来の追加。挿入先は兄弟の URL でドロップの瞬間に決め、非同期の取り出しを待つあいだの一覧の変化で index がずれないようにする。フォルダー行はドラッグできない（フォルダーは名前順のまま） |
...
| `ViewerWindowKind` | ビューア窓の種別（`.viewer` / `.slide`）。生成時に決まり以後変わらない（`let` で受ける）。`allowsSidebar` / `hasToolbar` / `joinsTabs` / `isRestorable` / `recordsUsageHistory` / `acceptsReopen` の 6 つの述語と `defaultContentSize`（`.viewer` は 1100×850、`.slide` は 1280×720 = 16:9）を持ち、**呼び出し側で `kind == .slide` と書かない**。`.slide` はプレゼン用のスライド窓で、サイドバー無し・ツールバー無し・タブ合流なし・セッション復元の対象外・利用履歴に残さない・再オープンの受け皿にならない。`joinsTabs` は**両方向をこの 1 つで決める**——既存のグループへ合流する側（`ViewerWindowChrome` の `tabbingMode`）と、この窓のグループへ他の窓が加わる側（`ViewerWindowOpenPolicy.effectiveDisposition`）。器の `tabbingMode = .disallowed` は**自動**タブ化しか止めず、`NSWindow.addTabbedWindow(_:ordered:)` の明示的な結合は通る（TASK-615）。`recordsUsageHistory` は「最近使った項目」と「最近使ったリポジトリ」の両方を 1 つの述語で扱い、判定は各履歴ストア（`RecentDocumentsStore.noteOpened(_:kind:)` は `kind` を必須引数で受け、`RecentRepositoryRecorder.recordIfNeeded(for:controller:)` は解決を始める前に弾く）の中に置く（TASK-610） |
...
| `PDFPageIndicatorModel` | PDF 面の「現在ページ / 総ページ数」。窓ごとに 1 個で、生成は `DocumentSurfaces` の 1 箇所だけ（`PDFFindModel` と同じ粒度・同じ置き場）。**面へは `PDFViewProxy` 越しに読むだけで何も書かない。** 値を溜めず、通知のたびに面から読み直すので、回転・倍率・文書の差し替えのどれでも次の通知で実態へ揃う。編集状態（`isEditing` / `draft`）と入力の検証（`pageJumpTarget(from:pageCount:)`）もここが持つ——View の `@State` に置くと SwiftUI の外から触れず、受け付けない入力の扱いをテストで固定できない。更新契機は `NSClipView` の bounds 変更と `.PDFViewDocumentChanged`——PDFKit の `.PDFViewPageChanged` は使わない（窓へ載せてもヘッドレスでは一度も飛ばず、`currentPage` も 0 のままでテストから測れない / TASK-578.1 の実測）。購読は `object: nil` で受けて発火時に同一性で絞る（スクロールビューは文書の差し替えで作り直されるため、インスタンスを固定すると無音で更新が止まる） |
| `PDFPageIndicator` | 現在ページを PDF の**左下**に重ねる常時表示。クリックすると数字入力へ変わり、ページ番号を打って確定するとそのページへ飛ぶ（TASK-578.2）。**`onTapGesture` ではなく `Button` を使う**——この面は AppKit がホストする `PDFView` の上に重なる SwiftUI で、実機ではタップジェスチャが合成クリックでも `AXPress` でも発火しなかった（右上の回転コントロールと同じく `Button` なら動く）。右上は回転と検索が排他で使っており、常時表示を足すと必ずどちらかと重なる。地は他のオーバーレイと同じ（`.regularMaterial` + `.separator` 枠）で、文字は `.secondary` に落とす。**総ページ数が 0 のときだけは自分で引っ込む**——「文書が無い」「面がまだ組み上がっていない」のどちらでも 0 になり、`1 / 0` を描いてしまうため |
| `ViewerMenuValidator.validateFocusTraversalItem` | サイドバーと本文のあいだのフォーカス移動（⌘← / ⌘→ / TASK-584）の有効判定。**Tab は使わない**——web 面では Tab / Shift+Tab がブラウザ既定の文書内リンク送りに使われており、奪うと Markdown 文書のリンクをキーボードで辿れなくなる。⌘← は畳んでいるときだけ無効（行が無いサイドバーへ送っても宛先が消えるだけ）。**⌘← のサイドバー側「上位フォルダーへ移動」は廃止した**——上へ出る手段は ⌘↑ と delete で足り、Finder も ⌘← を上位移動には使わない。⌘→ は能力で絞らない |
| `DocumentSurfaceOperating.focusSurface` | キーボードのフォーカスを**いま描いている面**へ移す。宛先の決定は `DocumentSurfaces.operating(on:)` に乗る。CLI と Quick Open で開いた窓は本文にフォーカスを置き（契機を知っている呼び出し元が指示する。ワイヤ表現の `CLIOpenOptions` には足さない）、サイドバー由来のオープンはサイドバーに残す（TASK-581） |
| `FocusClaimingTextField` | PDF の面に重なる SwiftUI で使う一行入力欄（AppKit の `NSTextField` を `NSViewRepresentable` で包む）。**SwiftUI の `TextField` + `@FocusState` では打ち込めない**——この面は AppKit がホストする `PDFView` の上に重なる SwiftUI で、`isInputFocused = true` を入れても窓の first responder は `ZoomingPDFView` のままだった（実測 / TASK-578.2）。窓へ入った 1 周後に `makeFirstResponder` を呼んで明示的に移し、Enter と Esc も AppKit の `doCommandBy` で受ける。**この面に置く入力欄は必ずこの型を通す**——ページ番号（TASK-578.2）と検索バー（TASK-579）で同じ穴が 2 回開いたため、個別修正をやめて 1 つに寄せた。**閉じた後の戻し先はこの型では決めない**（`PDFViewProxy.focusSurface` が面に決め打つ） |
...
表示仕様の本体は次の 2 文書にある（TASK-634 で分割）。

- [ファイル種別ごとの表示仕様](./file-type-display.md) — `.md` / `.mmd` / `.xml`（XSLT）/ PDF / CSV・TSV の扱い、印刷、キャンバスの所有者、種別ごとの表示幅
- [ビューアの操作 UI](./viewer-ui.md) — ズーム、キーボードスクロール、検索・文書内ジャンプの統合バー、表示モード切替、履歴、エラーパネル、ショートカット一覧、サイドバー
