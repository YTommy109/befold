...
befold/
├── App/
│   ├── BefoldApp.swift        # @main・Settings・メニュー定義
...
- ファイル消失時はグレース期間(1秒)後に `onFileGone` コールバックで通知し、ウィンドウを閉じる
...
| GitHub Pages | 独自ドメインを設定できる。URL が短く安定する（`https://degino.github.io/befold/appcast.xml`） | Pages 用ブランチまたはリポジトリの管理が追加で必要。リリース CI から Pages へのデプロイステップを別途組む必要がある |
...
```
https://github.com/user/befold/releases/latest/download/appcast.xml
```
...
- `YES` に設定するとユーザーの同意なく外部通信が発生するため、
  プライバシーの観点で避けるべきである
- befold はローカル完結のビューアアプリであり、ネットワーク通信はアップデート確認のみ。
