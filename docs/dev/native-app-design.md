...
| `BookmarkStore` / `BookmarkLibrary`（BefoldKit） | ブックマークの永続化。値は `BookmarkLibrary`（エントリ = 正規化パス + 任意の別名 + 所属フォルダーの名前列、フォルダー = 名前列 + 展開状態）で、UserDefaults キー `Bookmarks` に JSON で 1 つ。純粋な操作（追加・削除・別名・rename 追随）は値型 `BookmarkLibrary` にあり、`BookmarkStore` は「読む → 1 操作 → 書く」の薄い層。デコード済みの値をメモリに持ち、自分の書き込みで更新する——前提は「同一プロセスの writer はこのインスタンスだけ」（`AppStores` が 1 個、CLI は GUI 起動中は転送）。旧キー `BookmarkedPaths`（パス配列）は `init` で一度だけ移行し、移行の有無にかかわらず削除する（TASK-536.1）。GUI と CLI（`befold --bookmark`）が同じ実装を使う |
| `BookmarksMenuController` | Bookmarks メニューの一覧を表示直前に作り直す `NSMenuDelegate`。表示名は `BookmarkEntry.displayName`（別名 ?? ファイル名）で並べ、右列のパスは別名を付けても親ディレクトリのまま。**`FileReading` を持たない**（存在確認は `MissingBookmarksPruner` に閉じる） |
...
| `BookmarkManagerModel` / `BookmarkManagerView` | ブックマーク管理パネル（Bookmarks > ブックマークを編集…）。モデルは `BookmarkStore` のスナップショットを持ち、各操作はストアへ書いてから取り直す。パネルが key になったときも取り直す（窓側の ⌘D で変わった分を拾う）。**stat はしない**（欠落バッジを出さない）。文字入力は `.alert` に `TextField` を置く形（インライン編集は持たない） |
