...
- コードの「なぜ（WHY）」を書く。「何を（WHAT）」は明確な命名で伝える
  ...
  - **型・ファイルの削除・追加・リネームは、アーキテクチャ図とプロジェクト構成ツリーへの
    ...
    さらに、**型の削除・リネームは、`coding_rule.md` 内のコード例・命名例・テスト例・
...
- **テスト関数の上**: 何を検証しているかが関数名で不十分な場合に `///` で補足する
  - プロダクトコード節の「書かなくてよいコメント」はテストコードの `///` にも等しく適用される
...
- **スイート内のテストヘルパー（ファクトリ・ビルダー・判定関数）の可視性は既存ヘルパーと揃える**。
  ...
  パラメタライズ用ヘルパー型の可視性規定（前掲）と同じ原則——同一スイート内での一貫性——の
...
テストコード全体レビュー（2026-08-01）で顕在化した、CI の実行時間・安定性に効く 6 件。
...
#### 否定検証（発火しない・観測しない）は番兵優先、使えないときだけ固定待ち + 0.3 秒
...
- **番兵方式にも限界がある**: 番兵が担保するのは「配送機構が動いた」ことだけであり、
  ...
  劣化を、番兵方式自身が別の形で持ち込む。窓を閉じるには次のどちらかを取る:
  1. 対象の observer を番兵より**先に登録**する前提を、意図としてコメントへ明示する
     （登録順に呼ばれる実装依存であることを残す）
  2. 厳密にやるなら**2 回目の配送を観測**して 1 回目のディスパッチパス完了を確定させてから
     未観測を検証する（配送は直列化されるため、2 通目の観測は 1 通目の全 observer 呼び出し
     完了を含意する。実例: `DistributedAckWaiterIntegrationTests` はこの 2 段番兵方式を採用）
