...
- **行数閾値は型グループ単位で機械的に強制される**: `scripts/check-type-group-size.sh` が
  ...
  - **恒久例外**: どうしても閾値へ収まらないグループだけを
    ...
    列挙する。理由の無いエントリはスクリプトが弾く。**現在どのグループがいくつの上限で
    登録されているかはこの文書に写さない**（上限は実測値へ張り付けるため動く）。
    そのファイル自身を見ること
  ...
  - **例外行を消すときは、そこに書いた「理由」の行き先を決める。** 例外の 3 列目には
    ...
    同じ調査をやり直すことになる。移す先はコードの doc コメント（型のすぐ上）を第一候補、
    入り切らなければこの規約文書か ADR とする
    - 実例: `MainMenuBuilder` は TASK-585 で 403 行になった際に「メニュー項目の定義そのもので、
      責務で切っても分かれない」という理由つきで上限 420 の例外を登録したが、2 日後に 397 行へ
      戻った時点で行ごと削除された。判断が残っていなかったため TASK-547 が同じ調査を
      やり直し、同じ結論に達している

- **`MainMenuBuilder` は「メニュー項目の定義」以上には分割しない**（TASK-585 / TASK-547 で
  2 回実測した結論）。型グループが閾値へ近づくたびに分割したくなるが、次の 3 点で成立しない。
  - `@MainActor enum` の名前空間で、stored property・protocol 準拠・親からの注入クロージャが
    いずれも 0 個。上に挙げた「関心の同居」を測る指標がそもそも存在せず、残る軸は
    「どのメニューか」という並列分類だけ
  - `Foo+Bar.swift` は `Foo` グループへ畳まれるので、`+EditMenu.swift` 等へ切っても合算は
    1 行も減らない。緩むのは SwiftLint の `type_body_length` の方であって、グループ 400 ではない
  - 別の型名へ出すと、配布サイトのショートカット検証が読む `MainMenuBuilder*.swift` の glob
    （`site/vitest.config.ts` と `.github/workflows/site.yml` の paths）から外れる。同時に
    site 側も作り直さない限り、実装とページの突き合わせが片側だけ欠ける
  - したがって 400 行を超えたら、分割ではなく恒久例外へ上限と理由を登録する
