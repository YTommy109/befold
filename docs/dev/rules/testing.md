...
- **JavaScript**: Jest を使う。テストは `BefoldApp/viewer-test/` に置き、
  ...
  **Jest を通しても型は検査されない**（本体は `npm run typecheck:viewer`、テストは
  `viewer-test/tsconfig.json` を使う `npm run typecheck:viewer-test` が担当する）
...
逆に、競合を測らない普通のテストで個別のハンドルを await しないこと。サイドバーは
1 回の更新で 3 本のタスク（一覧・git 状態・基準ディレクトリ）を発行し、どれが反映を
運ぶかは絞り込みの ON/OFF で変わる。`SidebarNavigator.awaitSettled()` が 3 本まとめて
待つので、待ち先を選ばずに済む。個別のハンドルは上記の競合テスト専用。
