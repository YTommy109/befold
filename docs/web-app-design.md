...
**システム設計 Web アプリ** のアーキテクチャ設計書である。
もう一方（macOS ネイティブアプリ）は [native-app-design.md](./native-app-design.md) を参照。

- markdown + mermaid で書かれたシステム設計ドキュメントを管理・閲覧する Web アプリ
- **人向けのテキストエディタは提供しない**。ドキュメントの作成・更新は
  ブラウザ上の依頼 UI から AI（Claude API）に依頼して行う
- ストレージはファイルシステムではなく **PostgreSQL**
- フロントエンドは現行 mmdview と同じ **htmx + Jinja2 + mermaid.js** を踏襲する
...
FastAPI (uvicorn)
  ...
  │    └── event_bus         # ジョブ完了 → SSE 通知（mmdview から流用）
  └── SQLAlchemy 2 (async) + asyncpg
        ▼
     PostgreSQL
```

- 現行 mmdview の「FastAPI + htmx + SSE」構成をそのまま発展させる。
  pywebview・watchdog・PyInstaller はこのアプリでは使わない
...
<!-- derived-from ./superpowers/specs/2026-05-30-mmdview-design.md#表示 -->

現行 mmdview は「ファイル全文を `<pre class="mermaid">` に流し込み mermaid.js に
任せる」方式だが、本アプリは設計書（散文＋図）が主体のため markdown を正規に
レンダリングする。
...
mermaid.js の初期化設定は現行実装をそのまま引き継ぐ:
...
mermaid 構文エラー時は現行と同じく `mermaid.parseError` でエラーパネル
...
- HTML 返却ルートは htmx + Jinja2、API は JSON。現行のルーター分割
  （`routers/html.py` / `routers/events.py`）の構成方針を踏襲する
- SSE イベントは `job_finished:{job_id}` / `job_failed:{job_id}` /
  `document_updated:{document_id}` の 3 種。`event_bus.py` の
  「`asyncio.Queue` ＋ `call_soon_threadsafe`」実装を流用する
  （本アプリは全処理がイベントループ内のため `call_soon_threadsafe` は不要になり、
  さらに単純化できる）
...
- **メイン**: レンダリング済み設計書（スクロール可能）。
  ズーム機能（0.5〜2.0、`Cmd +/-`・`Ctrl + ホイール`、localStorage 永続化）は
  現行 viewer.html の実装を移植する
...
| 現行 mmdview | 本アプリでの扱い |
|---|---|
| htmx + Jinja2 + mermaid.js | そのまま採用（mermaid 初期化設定・エラーパネル含む） |
| SSE（sse-starlette）＋ event_bus | 通知対象を「ファイル変更」から「ジョブ完了」に変えて流用 |
| ズーム機能（viewer.html） | JS/hyperscript 実装を移植 |
| watchdog ファイル監視 | **廃止**（DB が真実の源泉。変更は自アプリ経由のみ） |
| pywebview / Apple Events / メニュー | **廃止**（ブラウザで利用） |
| PyInstaller バンドル | **廃止**（サーバーデプロイ） |
| window_state.json / recent_files.json | **廃止**（DB と URL で代替） |
| 自動アップデート機能 | **廃止**（サーバー側でデプロイ更新） |
...
| jinja2 / htmx / mermaid.js | テンプレート・UI（現行踏襲） |

**開発ツール**: uv / taskipy / ruff / ty / pytest（現行と同一構成）。
...
現行 mmdview のテスト規約（AAA スタイル・カバレッジ 80% 以上・unit 60 秒以内）を踏襲する。
