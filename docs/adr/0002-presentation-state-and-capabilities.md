...
この不変条件は `DiffDisplayPreference.isEnabled`（差分の ON/OFF をアプリ全体で
持っていた頃のプロパティ）の doc コメントにだけ書かれており、TASK-356 で
その持ち方をやめてプロパティを削除したときに一緒に失われた（TASK-371）。
コードの一箇所にしか存在しない不変条件は、その箇所が消えるときに一緒に消える。
...
## 状態の所在（ファイル単位 / アプリ全体 / 窓ごと）

<!-- derived-from #複数ウィンドウの不変条件 -->

前節の不変条件は表示モードについてのものだが、同じ問いは倍率・スクロール位置・
行番号表示・サイドバーの開閉など、ビューアが持つすべての状態に生じる。
表示モードごとに個別に決めると判断が場当たりになるため、**新しい状態を足すとき・
既存の状態の持ち方を変えるときは、まずこの分類のどれに当たるかを決める。**

### 分類を決める問い

> **同じファイルを 2 つのウィンドウで開いたとき、その値が食い違って見えたらバグか?**

- **バグ** → 文書の性質に属する。ファイルパス単位で永続化し、**全ウィンドウで一致させる**
- **バグではないが、全ウィンドウで揃っていてほしい** → 利用者の好みに属する。
  ファイルに紐付けず、アプリ全体で 1 インスタンスを注入して共有する
- **窓ごとに違ってよい** → 窓の操作履歴に属する。窓ごとに持ち、永続化も同期もしない

### 3 分類

| 分類 | 判定 | 持ち方 | 例 |
|---|---|---|---|
| 文書の性質 | その文書をどう読むかが値に表れ、窓が違うだけで変わるのはおかしい | `PathKeyedDictionary` 上のファイルパス単位のストアへ永続化し、変更時に同一パスの窓へミラーする | 表示モード（`DisplayModeStore`）、ズーム倍率（`ZoomStore`） |
| 利用者の好み | どのファイルかに依らない見え方の設定。ファイルに紐付けると「このファイルだけ設定が違う」という説明できない状態が作れてしまう | アプリ全体で 1 インスタンスを生成し、全ウィンドウへ注入して共有する | 差分レイアウト（`DiffDisplayPreference`）、検索オプション（`FindOptionsPreference`）、コードフォント（`CodeFontPreference`）、隠しファイル表示・変更ファイルのみ表示（`SidebarDisplayPreference`） |
| 窓の操作履歴 | その窓で何をしたかの記録。他の窓に持ち込むと操作していない窓が勝手に動く | 窓ごとのオブジェクト（`ViewerStore` / `ViewerWindowController` / `SidebarNavigator`）に持ち、永続化も同期もしない | cmd+U の戻り先（`sourceToggleReturn`）、戻る/進む履歴（`NavigationHistory`）、ディレクトリごとの選択記憶 |
...
- **UserDefaults に書くかどうかは分類を決めない。** 好みも文書の性質も永続化するし、
  操作履歴は永続化しない。決めるのは「キーが何に紐付くか」と「窓をまたいで揃えるか」。
- 窓ごとの永続状態（`SidebarStateStore` / `WindowFrameStore`）は、ウィンドウ ID ではなく
  **そのとき開いているファイルパスを窓の代理キーにしている**。同一ファイルを複数窓で
  開くとこの代理が崩れるため、新しい状態をここへ足す前に代理が成立するかを確かめる。

### 現状の適用結果（2026-08-09 時点）

基準を既存の状態へ当てると、表示モード以外に 3 件の逸脱がある。いずれも
「分類は決まるが持ち方がそれに従っていない」形で、修正は別タスクへ切り出した。

| 状態 | 分類 | 現状 | 追跡 |
|---|---|---|---|
| ズーム倍率 | 文書の性質 | ファイル単位で永続化しているが窓間ミラーの経路が無く、窓 A の変更は窓 B が `applyStoredZoom` を通るまで反映されない | TASK-388 |
| 行番号表示 | 利用者の好み | アプリ全体キー `ShowLineNumbers` へ書きながら、ライブ値は窓ごと（`ViewerStore.showLineNumbers`）。切り替えても他窓は再生成まで追従しない | TASK-389 |
| スクロール位置 | 窓の操作履歴の疑い | `(パス, モード)` 単位でアプリ全体共有。2 窓が独立に切り替えると同一キーへ後勝ちで上書きされ、別窓の位置が復元されうる | TASK-390 |
...
発火したため、3 本すべての現状を評価し決定を再検討した。**結論は「決定を維持する」。**
...
**維持する理由**: 窓間同期の要件は、規約 1（提示状態を 1 つの値で持つ）を弱めるどころか
それに支えられて成立した。同期すべき対象が 1 個の値だったため、同期は
「`setDisplayMode` の完了後にデリゲートで 1 回渡す」だけで済み、経路は 1 本に閉じている。
複数の Bool を持っていれば、同期する組み合わせと同期しない組み合わせの選択が生じていた。
リデューサ的な枠組み（トリップワイヤ 2）を必要とする分岐の増加も観測されていない。
したがってフレームワークは引き続き導入しない。

**同時に不足も確認した**: 窓間で揃えるべき状態とそうでない状態の線引きが、
表示モードについてしか書かれていなかった。これを「状態の所在」節として一般化し、
基準から外れている既存の 3 件（TASK-388 / 389 / 390）を切り出した。

再発火の条件を次のとおり引き直す。

1. （変更なし）段 1〜3 を終えても同種の回帰が再発する
2. （変更なし）状態遷移の分岐が手書きで追えない規模になる
3. トリップワイヤ 3 は**発火済みで、決定を維持したまま吸収した**。次にこれを問い直すのは、
   **「状態の所在」の 3 分類に収まらない状態が現れたとき**、または
   **窓ごとの状態を窓 ID で持つ必要が生じたとき**（ファイルパスによる窓の代理が破れたとき）
