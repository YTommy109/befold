...
PDF へ切り替えた直後の白紙を、可視ページを同期で焼いた静止画（`PDFSurfacePlaceholder`）で埋めた。

**前提が 1 つ崩れたところから始まった。** 実シナリオ（サイドバーの ↓）で、終点を「ページの
中身が画面に出た時刻」にして測り直すと、現在の版は 1.15.1 より**速かった**（中央値 52.3ms 対
99.1ms）。遅いのではなく**ばらつく**のが問題で、18 回中 3 回が 274〜286ms へ跳ねていた。
1.15.1 は 18/18 が 76〜108ms に収まり一度も跳ねない。前回の測定は 2 点で誤っていた——終点に
`PDFView.draw(_ dirtyRect:)`（面が塗られた時刻）を使っていたことと、再現に FileWatcher の
rename 追従を使って debounce 0.2s を測り込んでいたこと。

跳ねはアプリ側ではない。区間ログとサンプラを同じ時計で突き合わせると、跳ねた回もアプリ側の
仕事は +41.5ms で完了しており、差は PDFKit の非同期タイル描画の中だけにあった（メインスレッドは
詰まっていない）。タイルが載った瞬間を知る手段も無い（`PDFPageView` の layer は contents が
nil のまま、`draw(_ page:to:)` の override は super が @MainActor で呼べない）。

単純な案は先に試して実測で棄却した。(a) 1 ページ目を同期描画して PDFKit を温める→跳ねは残り
中央値 +9ms 悪化。(b) 倍率適用がタイル要求を無効化しているというレース仮説→遅い回と速い回で
`scaleFactor` の変更は 1 回ずつ・順序も同一で支持されない。

対処は「いつ載ったかを知ろうとせず、載っても困らない絵を先に置く」。外すのは面が動く事実だけ
（スクロール・倍率・回転・寸法変化・次の差し替え）で、これらは `PDFSurfaceLayout` に収斂して
いるので列挙漏れが起きない。上限 0.4 秒は保険で正しさには効かない。

結果は 18/18 が 25.3〜50.4ms（中央値 41.2ms）で跳ねゼロ。中央値・最小・最大のすべてで
対処前と 1.15.1 の両方を下回った。

`swift test` 1812 tests / 294 suites 全通過（`PDFSurfacePlaceholderTests` 8 件を追加）、
swiftlint はベースライン差分ゼロ、計装は全撤去。
