...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 累計 / 当日 / 日毎の推移 / 時間帯分布 の 4 領域が、それぞれ何を数えた値か画面上のラベルから一意に読み取れる
- [x] #2 「日次」と表示される指標が実際に当日のみを集計している（全期間の延べ数を日次と表示している現状が解消されている）
- [x] #3 日付・時刻の基準タイムゾーンが全指標で JST に統一され、JST 基準である旨が画面に明示されている
- [x] #4 日毎の推移と時間帯分布がグラフとして描画される
- [x] #5 SSE によるライブ更新が再設計後の構成でも機能する
<!-- AC:END -->
...
2026-08-08 サブタスク 359.1 / 359.2 / 359.3 をすべて完了。集計層の再設計と JST 統一（359.1）、6 セクション構成への組み直しと不連続注記（359.2）、インライン SVG によるグラフ描画（359.3）。検証は site で npx vitest run 74 passed / 6 files、npx tsc --noEmit エラーなし。未検証: 実ブラウザでの表示確認（セクション間の余白、24 本棒グラフのラベル可読性）。
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
analytics ダッシュボードを再設計した。集計を累計 / 当日 / 日別推移 / 時間帯分布の 4 つに分け、全指標の日付・時刻基準を JST へ統一（visitor_day のハッシュ日付を含む）。ページを 6 セクションへ組み直して各見出しに集計期間を明記し、日毎の推移と時間帯分布をインライン SVG のグラフで描画した。グラフはサーバ側描画にすることで SSE の innerHTML 全置換後も再描画フック無しで残る。vitest 74 件 pass、tsc エラーなし。
