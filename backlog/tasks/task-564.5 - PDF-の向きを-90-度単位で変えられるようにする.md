...
- [x] #4 回転の操作が PDF の面に重ねたコントロールから届き、PDF 以外では出ない（メニューには置かない）
- [x] #5 アイコンだけのボタンに読み上げ名と tooltip が日本語・英語の両方で付いている
- [x] #6 別ファイルへ移って戻ったときに回転状態が保たれ、ウィンドウを閉じるとリセットされる
<!-- AC:END -->
...
## 回転の入口をメニューからオーバーレイへ移した（2026-08-29 / ユーザー指摘）

「回転はメニューではなく、アイコンで右上にオーバーレイしたものがいい」との指摘を受けて
入口を移設した。**その面を見ているときにしか意味が無い操作**なので、対象（いま出ている
ページ）の隣に置くほうが視線を動かさずに済む。viewer 側のダイアグラム個別ズームが
図の隅にコントロールを置いているのと同じ考え方。

- 追加: `PDFRotationOverlay`（`rotate.left` / `rotate.right`）。表示条件は PDF の面と
  **同じ 1 つの条件**（`DocumentSurfaceStack.showsPDF`）で決める。
- 撤去: View メニューの「右に回転」「左に回転」、⌘R / ⇧⌘R、`rotateDocument(_:)`、
  `ViewerMenuValidator` の回転分岐、site の期待ショートカット表の 2 行。
- `PDFSurfaceActions` を新設し、PDF 面まわりの受け渡し（倍率の通知・回転の要求）を
  1 つの値へまとめた。クロージャを 1 つずつ View の引数へ足すと注入が 3 つを超えるため
  （`docs/dev/rules/product-code.md` の責務分離節）。

**キーボードショートカットは無くなった。** メニュー項目がキー等価の置き場だったため、
移設で ⌘R / ⇧⌘R も消えている。アイコンだけのボタンには読み上げ名と tooltip を
日英で付けてあるので操作の発見と VoiceOver は成り立つが、キーボードだけで回す手段は
現状ない。必要なら別途起票する（オーバーレイのボタンへ `.keyboardShortcut` を付ける形が
考えられるが、`NSHostingView` 配下で効くかは未確認）。

AC #4 / #5 は実態に合わなくなったため書き換えた（旧: メニュー項目とショートカットの
両方から届くこと / ヘルプの一覧に載ること）。
<!-- SECTION:NOTES:END -->
