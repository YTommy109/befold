...
- **行あたりの正規表現は言語ごとに 1 本へ結合する。** `collect` は
  ...
  呼ばれるため
- **JS/TS のメソッド短縮記法（`foo() {`）は、名前の位置の予約語を否定先読みで
  外して拾う（TASK-485.23 で追加）。** `if (x) {` / `while (x) {` /
  `} catch (e) {` と行の形が同じなので、**分かれ目は予約語リストだけ**になる。
  取りこぼしはそのまま誤検出になるため、制御構文を拾わないことを
  `viewer-main-jump-function.test.js` が固定している。ここでもトークンは
  使っておらず、上の決定（トークンは除外にだけ使う）は変えていない
