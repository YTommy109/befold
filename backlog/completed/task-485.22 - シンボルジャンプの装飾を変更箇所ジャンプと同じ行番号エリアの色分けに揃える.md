...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 見出しジャンプのアクティブ箇所が枠線（outline）で囲まれなくなっている
- [x] #2 候補とアクティブが「薄い／はっきり」の濃淡で区別でき、変更箇所ジャンプと同じ読み方になっている
- [x] #3 行番号ガターが無い表示（Markdown レンダリング表示）でも候補とアクティブが判別でき、どう扱うかの判断が Notes に記録されている
- [x] #4 差分表示の変更箇所ジャンプの見た目は現状から変わっていない
- [x] #5 ライト／ダーク両方で候補とアクティブのコントラストを確認した記録がある
<!-- AC:END -->
...
Description が挙げていた (a) 擬似ガター新設 / (b) ガターのある表示に限定、の
どちらでもなく、**ユーザーの指示により「行番号ガターが無いレンダリング表示では
検索とまったく同じ装飾にする」**を採った（2026-08-24）。

結果として装飾は 2 系統だけになる。

| | 候補 `.mmd-jump-target` | 現在位置 `.mmd-jump-current` |
|---|---|---|
| 差分表示以外（レンダリング見出し・ソース行） | `--hit-bg`（検索の候補ヒットと同じ薄い地色） | `--accent` の地色 + 白文字（検索の現在ヒットと同じ） |
| 差分表示 | 装飾なし（打ち消し） | 左端セルの縦帯 `inset 3px 0 0 var(--accent)`（現状のまま） |

- `mark.mmd-find-match` が持っていた生値 `rgba(255,213,0,0.55)` を `--hit-bg` トークンへ
  括り出し、検索とジャンプで共有した。片方だけ色を変えると読み方がずれるため、
  値の置き場を 1 つにしてある（`style.css:55-57`）
- 枠線（`outline: 2px solid var(--accent)`）は撤去した（AC#1）
- 差分側は `.diff-table` スコープ（詳細度 0,2,0）で地色を打ち消しているので、
  `.diff-add` / `.diff-del` の地色と縦帯は現状から変わらない（AC#4）
- 擬似ガター (a) を採らなかったのは、markdown-body へ新しい左領域を足すと
  本文の左位置が動き、バーの開閉で紙面がずれるため

## 検証

- **ライト／ダーク両方のスクリーンショット実測**（AC#5）。`style.css` を実 file URL で
  `<link>` した独立ハーネス HTML を `WKWebView.loadFileURL` で読み、
  `appearance = .aqua / .darkAqua` を切り替えて `takeSnapshot` で PNG 化した
  （メモリ webview-css-snapshot-harness の方法）。ハーネスには見出し候補／現在位置、
  比較用の検索 `<mark>` 2 種、差分表の候補行／現在行を並べてある。
  - ライト: 候補 = 淡い黄、現在位置 = 青地に白文字、いずれも検索の `<mark>` と同一に見える。
    差分表は地色（赤／緑）と左端の青い縦帯が保たれている
  - ダーク: 候補は暗い地に対して黄が沈んだオリーブになるが本文（明るい文字）と
    十分に分離する。現在位置は `--accent`(#4aa3ff) 地 + 白文字で、**検索の現在ヒットと同じ
    コントラスト**。ここは検索側から引き継いだ既存の見え方で、変えるなら検索と一緒に
    変える話になるため本タスクでは揃えるに留めた
  - ハーネス HTML はスクリーンショット取得後に削除済み（リポジトリには残していない）
- `npx jest BefoldKit/Resources/__tests__/viewer-main-jump.test.js` → 69 passed。
  クラス名（`mmd-jump-target` / `mmd-jump-current`）は変えていないので JS 側の変更は無い
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
見出しジャンプの装飾を検索の <mark> と同じ地色（候補=--hit-bg / 現在位置=--accent+白文字）へ揃え、outline の枠線を撤去した。検索の候補ヒット色は --hit-bg トークンへ括り出して共有。差分表示は .diff-table スコープで打ち消し、地色と左端の縦帯を現状のまま維持。ライト／ダーク両方を WKWebView takeSnapshot で実測し、viewer-main-jump.test.js 69 件が通ることを確認した。
