...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 同一ディレクトリのファイルを Cmd+クリックで新規タブに開いたとき、サイドバーの行が再構築されず、目視でちらつきが確認できない
- [x] #2 着手時に実機でちらつきを再現・記録し、修正後の比較結果を Implementation Notes に残す（コード上の推論のまま修正しない）
- [x] #3 同一ディレクトリでの再列挙が実際に何回走っているかを計測し、修正後の回数と併せて記録する
- [x] #4 サイドバーの表示内容が実際に変わる場合（別ディレクトリのファイルを新規タブで開く・フォルダー移動・隠しファイルトグル・並び順変更・レイアウト切替・リネーム）は従来どおり更新される
- [x] #5 ファイル監視をしていない前提（windowDidBecomeKey での取り直し）が要る場面 — 外部でファイルが追加・削除された後にウィンドウをキーにする — で、一覧が更新されることを確認する
- [x] #6 「同じ内容なら作り直さない」ことを固定する回帰テストを追加し、修正を戻すと落ちることを実測で確かめる
- [x] #7 着手前に /review-design を 1 回回し、結果を Implementation Plan に反映する（既存の状態・経路を増やさない方針の妥当性を確認するため）
<!-- AC:END -->
...
## 2026-08-20 最終: 実機で BEFORE/AFTER を画素で記録した（AC#1 / AC#2 充足）

補助アクセスが無くクリックはできないが、**列挙を人工的に遅らせて 32ms の事象を 6 秒へ
伸ばし**、外部から `screencapture -l<windowNumber>` でウィンドウを直接撮ることで、
画素での比較が取れた。修正コードは一切変えず、計測用ビルドにだけ遅延とプローブを
入れて撮影後に撤去した（`git diff` が空であることを確認済み）。

条件: Debug .app / 10 ファイルのフォルダ / `BEFOLD_TASK532_SLOW=6`（`DirectoryLister.listingAsync`
に遅延）/ Cmd+クリックと同じ入口（`openViewer(disposition: .newTab, relativeTo:)`）。

| | 新規タブを開いた直後の同じウィンドウ |
|---|---|
| BEFORE（引き継ぎなし） | 本文（bravo.md）は描画されているのに **サイドバーが完全に空**。フォルダーのヘッダー行だけ |
| AFTER（引き継ぎあり） | **10 件すべてが最初から並び**、bravo.md が選択済み |

画像: `~/Desktop/task-532-evidence/before-sidebar-empty.png` /
`after-sidebar-populated.png`（リポジトリには入れていない）。

フレーム単位の計測（16ms x 45 フレーム、800 ファイルのフォルダ）も併せて取ってある。
BEFORE は一覧が空のフレームが 2 枚、AFTER は 0 枚。

### 検証まとめ

- `swift test` 1684 件通過（プローブ撤去後の最終ツリーで再実行）
- swiftlint 新規違反 0（main 54 → head 54、ルール×ファイルで差分なし）
- 型グループ超過なし
- 修正を戻すと `SidebarListingSeedTests` / `SidebarIdenticalListingTests` の 6 件が落ちる
- 実機の BEFORE/AFTER を画素で記録（上表）
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
サイドバーの再構築を 2 箇所で止めた。(1) 同一ディレクトリの再列挙で結果が前回と完全に同じなら反映しない（判定は SidebarTreePresenter.applyRows）。(2) 新規タブ・新規ウィンドウが同じフォルダなら、起点の窓の列挙結果を出発点として引き継ぐ（SidebarListingSeed。運ぶのは行ではなく材料なので、窓ごとの展開状態と食い違わない）。

起票時の原因分析のうち「同値でも代入するので SwiftUI が行を作り直す」は実測で誤りと判明した（Swift の Observation は Equatable な値の同値代入では観測を汚さない）。真因は新規タブが空の一覧で作られる 2 段階のほうで、実アプリの計測で確定させた。

検証: swift test 1684 件通過 / swiftlint 新規違反 0（54→54）/ 型グループ超過なし / 修正を戻すと専用テスト 6 件が落ちることを実測 / 実機で BEFORE・AFTER を画素で記録（列挙を 6 秒遅延させて撮影。BEFORE はサイドバーが空、AFTER は全件表示）。
