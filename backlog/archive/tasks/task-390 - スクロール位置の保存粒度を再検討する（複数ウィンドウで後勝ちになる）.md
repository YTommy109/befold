...
<!-- SECTION:NOTES:BEGIN -->
TASK-388 へ統合（2026-08-09）。TASK-382 の再検討で『文書の状態は窓のライブ値を優先し、生きている間は保存値を読み直さない』という 1 本の原則に決まったため、スクロール位置・ズーム倍率・表示モードを個別タスクで直すと同型の穴が残る。スクロール位置の (パス, モード) 粒度は「次に開くときの既定値」としては妥当であり、粒度変更は不要という結論になった（後勝ちで別窓の位置が復元される問題は、生きている窓が保存値を読み直さなくなることで解消する）。
<!-- SECTION:NOTES:END -->
