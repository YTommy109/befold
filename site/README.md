...
- OS 別・接続元組織別の内訳は、指標ごとに独立した表として出す
  （例: 「ページアクセス: OS 別」「ダウンロード（LP）: 接続元組織別」）。
  3 種を合算した「OS 別」「接続元組織別」の表は**廃止した**（置き換え）。
- 国別は「どこから来訪したか」を見る軸で、指標を分けても読み取れる情報が
  増えないため 3 種合算のまま残している。
- 参照元別は**合算と「ダウンロード（LP）」の 2 つ**を出す。合算だけにしていた
  のは同じ理由だったが、`?ref=` が「ダウンロードが始まった面」を運ぶように
  なった（TASK-549）ことで前提が変わった。合算の表が答えるのは「どこから
  来訪したか」で、ダウンロードに絞った表が答えるのは「どの面のボタンから
  押されたか」——母集団も問いも違う。
  - サイト内のダウンロードリンクは `downloadHref()`（`src/views/shared.tsx`）
    だけで組み立て、`?ref=` を必ず載せる。素の `/download` でリンクを書くと
    Referer が自ホストになって referrer が null になり、集計から行ごと消える。
    迂回した書き方は `test/public.test.ts` が落とす。
  - ダウンロード側の表だけは `?ref=` の無い行も `未記録` として並べる。黙って
