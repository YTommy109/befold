...
医療費/
  README.md                     ← このファイル
  CLAUDE.md                     ← LLM 向けの入口
  providers.md                  ← 医療機関の一覧（名称・URL・住所・電話）
...
ルート直下に置くと、年数を重ねるほど README.md や providers.md といった
...
1. `Inbox/` の未処理ファイルを読み、日付・医療機関・金額・対象者・区分を抽出する。
   初めて出てくる支払先なら、領収書に印字された住所・電話番号を `providers.md` に追加する
...
- **作業環境** … クラウド上に、その会話専用に用意される Linux 環境。
  Mac とは別のコンピュータで、会話が終われば破棄される。`pdfimages` や Python が入っていて、
...
- **見るのはビューア、直すのはテキストエディタ。** 表計算ソフトで開いて保存し直すと
  書式（先頭ゼロ、日付形式）が変わることがある
...
`data/medical-YYYY-氏名.tsv` の `provider` に出てくる医療機関・薬局の一覧。
**利用者ごとにまとめ**、その下に支払先の見出しを立てて箇条書きで書く。

以下は書式の記入例で、医療機関名・住所・電話番号は実在しません。
...
- `##` は利用者、`###` は支払先
- 支払先の名称は `data/medical-YYYY-氏名.tsv` の `provider` と**完全に一致させる**
- 項目は `URL` / `住所` / `電話` / `出典`。分からないものは行ごと省く（推測で埋めない）
- 複数人が利用する支払先が出てきたら `## 共通` の節に移す

### 利用者ごとにまとめる理由

生活圏が違えばかかる医療機関も別になる。混ぜると一覧が長くなるだけで、
どちらの通院先か分からない。利用者で分けておけば、その人の通院先だけを見渡せる。

住所と電話番号は**領収書に印字されているので、取り込みのついでに拾える**。
新しい支払先の領収書を読んだら、その場で `providers.md` に追記する。
医療費控除の明細書に住所の記載は要らないが、税務署から支払先を問われたときや、
同名の別店舗と区別するときに効く。

年をまたいで使い回すので、集計表と違ってファイルは年で分けない。
集計表側に住所や URL を持たせると同じ情報が何度も並ぶので、名称をキーにこちらへ寄せる。

### これだけ Markdown にする理由

1 支払先あたり数項目しかなく、しかも多くが空欄になる。
表にすると空セルばかりの横長になって読みにくい。
集計表と違って機械的に合計する対象でもないので、見出し＋箇条書きの方が素直。
...
- **LLM からこのフォルダが見えるのは、Mac が起きていてデスクトップアプリが繋がっているときだけ。**
  出先から Mac を閉じたまま集計を頼みたい場面が増えるなら、集計表だけを別の場所に
  置く構成に変えることを検討する
- クラウドストレージの「ストレージを最適化」でファイル実体がローカルに無い場合、
  初回アクセスでダウンロード待ちが入ることがある。動作が遅いときはこれが理由
