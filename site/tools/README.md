...
- クライアント種別に `bot:ClaudeBot` / `bot:GPTBot` / `bot:Googlebot` / `bot:other` を
  混ぜてある。AI クローラの内訳が実運用でどう見えるかを確かめるため。接頭辞なしの
  `other` も 1 つ入れてあり、これは分類（ADR 0004）の適用前に記録された行を模したもの
  ——人間側に数えられるのが正しい挙動なので、ダッシュボードでそれを確認できる
