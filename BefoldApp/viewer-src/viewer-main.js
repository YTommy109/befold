// viewer.js が提供する純粋ロジック。ESM 化前は同一グローバルへ順次評価される
// 前提の裸参照だった（TASK-432.2）。付け忘れは eslint の no-undef で検出する。
import {
  BASE_SCALE,
  ...
  function _mmdInitMarkdown() {
    ...
    // 無害化する(sanitizeRenderedHtml は viewer.js の純粋関数、実処理は DOMPurify に委譲)。
