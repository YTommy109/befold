...
// 一致範囲をテキストノードごとの区間へ割り戻したもの。path は一致したパス全体。
...
function isLocalPathHref(href: string | null | undefined): boolean {
  if (!href) {
    return false;
  }
  if (href.charAt(0) === '#') {
...
//
// パス構成要素は \p{L}\p{N} も許可し、日本語ファイル名(20_経理/…)を拾えるようにする。
// ただし各セグメントの先頭 1 文字だけは \w のまま(GitHub Issue #653)。
// 日本語文はスペース区切りが無いため、先頭まで Unicode 化すると地の文の途中から
// マッチが始まり、直前の文章ごと「パス」として飲み込む(そのため20_経理/… のように)。
// 先頭を \w に固定すると、その前の文字が \w でない限りそこから先の日本語文へは
// マッチが伸びない。
var _PATH_RE =
  /(?:(?<![/\w.])(?:\/?\.\.?\/[\w.\p{L}\p{N}/-]+|[\w.-][\w.\p{L}\p{N}-]*\/[\w.\p{L}\p{N}/-]+)|(?:^|(?<=\s))\/[\w.\p{L}\p{N}/-]+)(?:\.(?:swift|md|mmd|ts|tsx|js|jsx|py|rb|go|rs|java|kt|c|cpp|h|hpp|json|yaml|yml|toml|txt|html|css|sh))(?::\d+)*/gu;
...
function _annotatePathRefs(): void {
...
function _isSkippedElement(el: Element): boolean {
...
function _annotateUnit(el: Element): void {
  var nodes: Text[] = [];
...
function _annotateTextNodes(nodes: Text[]): void {
  ...
  var segments: Record<number, PathSegment[]> | null = null;
...
function _replaceWithSegments(node: Text, segments: PathSegment[]): void {
...
var _mmdPendingRefBatches: RefTarget[][] = [];

// 分類済み(pending/解決済み/解決失敗)の参照は再収集しない。
function _mmdIsClassifiedRef(el: Element): boolean {
  return (
...
function _mmdResolveReferences(): void {
  ...
  wrap.querySelectorAll<HTMLAnchorElement>('a[href]').forEach(function (a) {
  ...
  wrap.querySelectorAll<HTMLElement>('.befold-path-ref').forEach(function (s) {
...
function _mmdApplyResolvedReferences(map: Record<string, string> | null | undefined): void {
...
function _mmdInvalidatePendingRefs(): void {
