...
// 受け取る値は 4 通りある。Swift の注入は number、エンコード失敗時の注入は null
// (ViewerBridge.defaultingFallback)、未注入は undefined、文字列も許容する
// （viewer.test.ts が '13' / 'abc' で呼んでいる）。null は String() で 'null' になり NaN へ縮退する。
...
// そのまま変わらない。
function markdownFontSize(raw: number | string | null | undefined): number {
