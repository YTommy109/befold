...
enum ViewerDisplayOptionsApplier {
    ...
    static func apply(
        ...
        // その区別は `applyCLIOverride` / `apply` のメソッド名が持つ(TASK-604.6)。
        if options.sortOrder != nil {
            controller.fileListModel.display.applyCLIOverride(sortOrder: options.viewerSortOrder)
        ...
        if hiddenFiles != nil, hiddenFiles != controller.fileListModel.display.showHiddenFiles {
            controller.fileListModel.display.applyCLIOverride(showHiddenFiles: hiddenFiles == true)
            controller.sidebar.refreshFileList()
        }
