...
/// ファイル毎に**永続化する**表示状態(倍率・サイドバー開閉)の束。
...
final class PerFileStateStore {
    ...
    /// - Parameter defaults: 各ストア(zoom / sidebar)の
    ...
    init(zoom: ZoomStore, sidebar: SidebarStateStore) {
        self.zoom = zoom
        self.sidebar = sidebar
    }

    /// ファイルの rename / move に伴い、旧パスの永続表示状態(倍率・サイドバー開閉状態)を
    /// 新パスへまとめて引き継ぐ。ウィンドウの寸法はファイル単位で持たないので対象外(TASK-583)。
