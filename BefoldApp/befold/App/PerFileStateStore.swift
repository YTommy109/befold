...
/// ファイル毎に永続化する表示状態(倍率・表示モード・スクロール位置)の束。
...
final class PerFileStateStore {
    ...
    /// - Parameter defaults: 各ストア(zoom / displayMode / scrollPosition / sidebar / windowFrame)の
    ...
    init(
        zoom: ZoomStore, displayMode: DisplayModeStore, scrollPosition: ScrollPositionStore,
    ...
    /// ファイルの rename / move に伴い、旧パスの全状態(倍率・表示モード・
    ...
    func migrate(from oldURL: URL, to newURL: URL) {
        ...
        displayMode.migrateDisplayMode(from: oldURL, to: newURL)
