...
///
/// - `ViewerWindowAssembler.makeSidebarGitStatusLoader(_:)`
...
/// - `ModeSegments.all`
///   — ツールバーのモード切替セグメントに差分を並べるか。無効時は 2 セグメント。
///     差分レイアウトの切替は独立アイテムではなく差分セグメントの再クリックが担うため、
///     ツールバー構成（`ViewerToolbarController.layout`）はゲートで変わらない。
