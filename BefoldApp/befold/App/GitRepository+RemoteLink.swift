...
/// ファイルを指すリモート（GitHub / GitLab / Bitbucket）のリンク解決。
/// `GitRepository` 本体から切り出しているのは、git の読み取り一般ではなく
/// 「サイドバーのメニュー 1 項目のための組み立て」だから。
extension GitRepository {
    /// origin の名前。リンクはこのリモートだけを見る。
    private static let originRemoteName = "origin"

    /// ファイルを指すリンク。ホスト名をメニュー文言へ差し込むため、URL だけでなく
    /// どのホストで解決したかも返す。
    struct RemoteFileLink: Equatable, Sendable {
        var forge: RemoteForge
        var url: URL
    }

    /// `url` が指すファイルのリモートリンク。作れなければ nil。
    ///
    /// nil に畳む理由は区別しない（git 管理外 / libgit2 で開けない / origin が無い /
    /// 対応外のホスト / detached HEAD）。呼び出し側の扱いはどれも「メニュー項目を
    ...
    /// ことは `GitRepositoryRemoteLinkTests` が固定する。
    ///
    /// 同期に libgit2 を開くため、**一覧の行ごとに呼ばない**こと。呼び出しは右クリックで
    /// メニューを組み立てる時点の 1 回に限る（`SidebarContextMenu`）。
    func remoteFileLink(forFileAt url: URL) -> RemoteFileLink? {
        let outcome = GitLibrary.withRepository(at: url.deletingLastPathComponent()) { repository -> RemoteFileLink? in
            guard let workdir = Self.workdirURL(of: repository),
                  ...
                  let link = resolved.forge.blobURL(
                      slug: resolved.slug, branch: branch, relativePath: relativePath
                  )
            else { return nil }
            return RemoteFileLink(forge: resolved.forge, url: link)
        }
