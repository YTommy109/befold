...
/// 同じリポジトリを開く N ウィンドウで `git ls-files` の実行と索引の実体を 1 つに畳むため。
///
/// ロックは目的ごとに 3 つに分ける。`git` subprocess を待つ間に握るのは
/// **キーごとのロック(`dirLocks` / `rootLocks`)だけ**で、キャッシュ本体を守る
/// `stateLock` は辞書の読み書きの間しか握らない。こうしないと 1 つの遅い `git ls-files` が
/// 無関係なリポジトリのウィンドウまで最大 15 秒(GitCommandRunner の timeout 10 秒 +
/// terminationGrace 5 秒)ブロックする。キーごとに分けることで、共有の狙いである
/// 「同一リポジトリでの実行の重複排除」は保ったまま、別リポジトリ同士は互いに待たない。
