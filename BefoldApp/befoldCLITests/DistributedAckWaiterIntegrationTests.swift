...
/// 長時間占有していると、その間 ACK は配送されず秒オーダーで止まりうる
/// (2 段番兵を試したとき、1 通目は成功し 2 通目だけ一貫してタイムアウトする現象を
/// 観測した。単独実行では再現せず、`.serialized` でも解消しなかったことから、
/// スイート内ではなく他スイートとの競合が原因と判断した)。そのため肯定待機は
/// 共有ヘルパーの既定予算(`testTimeoutSeconds(fallback: 15)`)を使い、
/// 待機の本数(番兵の段数)を増やさない設計にとどめる。
@Suite(testTimeLimit())
