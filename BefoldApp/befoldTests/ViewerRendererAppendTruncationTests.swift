...
struct ViewerRendererAppendTruncationTests {
    private static let renderedTruncation = ViewerRenderer.TruncationState(
        isTruncated: true, lineCount: 100, failed: false
    )
    private static let incomingTruncation = ViewerRenderer.TruncationState(
    ...
    private static func makeRenderer(_ webView: WKWebView) -> ViewerRenderer {
        ...
        renderer.rendered = ViewerRenderer.RenderedStateMirror(
            contentRevision: 5, fileType: .markdown, filePath: URL(fileURLWithPath: "/tmp/task417.md"),
            showLineNumbers: false, isSourceMode: false,
            truncation: renderedTruncation, diffState: ViewerRenderer.DiffState.none
        )
        return renderer
    }

    private static func makeRequest(generation: Int) -> ViewerRenderer.AppendRequest {
        ViewerRenderer.AppendRequest(
    ...
    func supersededAppendSendsNothing() async {
        ...
        await renderer.applyAppend(webView: webView, request: Self.makeRequest(generation: 8))
    ...
    func currentAppendSendsTruncationAndRecordsIt() async {
        ...
        await renderer.applyAppend(webView: webView, request: Self.makeRequest(generation: 9))
