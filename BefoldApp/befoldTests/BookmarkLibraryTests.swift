...
struct BookmarkLibraryTests {
    ...
    /// アイコンは拡張子だけで決まる(ブックマーク先に触れない)。拡張子の無い名前は汎用の書類。
    @Test(arguments: [
        (path: "/mock/docs/note.md", type: UTType(filenameExtension: "md")!),
        (path: "/mock/docs/diagram.mmd", type: UTType(filenameExtension: "mmd")!),
        (path: "/mock/docs/Makefile", type: UTType.data),
        (path: "/mock/docs", type: UTType.data),
    ])
    func iconTypeUsesExtensionOnly(path: String, type: UTType) {
        #expect(BookmarkEntry(path: path).iconType == type)
    }
