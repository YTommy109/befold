...
struct PDFFindModelTests {
    ...
    private func makeModel(
        pageCount: Int, caseSensitive: Bool = false
    ) -> (PDFFindModel, ZoomingPDFView, PDFViewProxy) {
        ...
        let model = PDFFindModel(pdfViewProxy: proxy, caseSensitive: { caseSensitive })
        return (model, pdfView, proxy)
    }
    ...
    func collectsMatchesAndSelectsFirst() async {
        let (model, _, _) = makeModel(pageCount: 5)
        model.open()
    ...
    func cyclesThroughMatches() async {
        let (model, _, _) = makeModel(pageCount: 3)
        model.open()
        model.setQuery("needle")
        await waitForSearch(model)
    ...
    func scannedPDFYieldsNoMatches() async {
           ...
           let context = CGContext(consumer: consumer, mediaBox: &box, nil)
        {
            ...
            context.fill(CGRect(x: 100, y: 100, width: 200, height: 200)) // 図形だけ
            context.endPage()
            context.closePDF()
    ...
    func lateMatchForAnotherQueryIsRejected() throws {
        let (model, pdfView, _) = makeModel(pageCount: 3)
