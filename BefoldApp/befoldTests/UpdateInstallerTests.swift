...
struct UpdateInstallerTests {
    ...
    private func makeScript() -> String {
        UpdateInstaller.updaterScript(
            appInDMG: "/Volumes/befold v1.2.0/befold.app",
            installedApp: "/Applications/befold.app",
            mountPoint: "/Volumes/befold v1.2.0",
            dmgPath: "/tmp/befold-update.dmg",
            pid: 12345,
            logPath: "/Users/dev/Library/Logs/befold-updater.log"
        )
    }

    @Test
    func installedAppURLAcceptsAppBundle() {
        let bundle = URL(fileURLWithPath: "/Applications/befold.app")
        #expect(UpdateInstaller.installedAppURL(bundleURL: bundle) == bundle)
    }

    @Test
    func installedAppURLRejectsDevBuildDirectory() {
        let devDir = URL(fileURLWithPath: "/Users/dev/befold/.build/debug")
    ...
    func updaterScriptContainsAllSteps() {
        ...
        #expect(script.contains(#"cp -R '/Volumes/befold v1.2.0/befold.app' '/Applications/befold.app.update'"#))
        #expect(script.contains(#"mv '/Applications/befold.app.update' '/Applications/befold.app'"#))
        #expect(script.contains(#"hdiutil detach '/Volumes/befold v1.2.0' -force"#))
        #expect(script.contains(#"rm -f '/tmp/befold-update.dmg'"#))
        #expect(script.contains(#"xattr -dr com.apple.quarantine '/Applications/befold.app'"#))
        #expect(script.contains(#"open '/Applications/befold.app'"#))
    ...
    func updaterScriptRedirectsOutputToLogFile() {
        ...
        #expect(script.contains(#"exec >> '/Users/dev/Library/Logs/befold-updater.log' 2>&1"#))
    }
    ...
    func updaterScriptCopiesToStagingBeforeRemovingInstalledApp() throws {
        ...
        let copy = try #require(
            script.range(of: #"cp -R '/Volumes/befold v1.2.0/befold.app'"#)
        )
        let removeInstalled = try #require(
            // 末尾のクォートまで含めることでステージング(….app.update)の削除とは区別する
            script.range(of: #"rm -rf '/Applications/befold.app'"#)
        )
    ...
    func updaterScriptKeepsInstalledAppWhenCopyFails() throws {
        ...
        let removeInstalled = try #require(script.range(of: #"rm -rf '/Applications/befold.app'"#))
        ...
        #expect(script.range(of: #"rm -rf '/Applications/befold.app'"#, range: afterElse) == nil)
        // open は分岐の外(常に実行)にあること
        let open = try #require(script.range(of: #"open '/Applications/befold.app'"#))
    ...
    func updaterScriptSafelyQuotesSpecialCharacters() {
        let evil = #"/Volumes/we"ir$d`whoami`/befold.app"#
        ...
        #expect(script.contains(#"'/Volumes/we"ir$d`whoami`/befold.app'"#))
    ...
    func updaterScriptEscapesSingleQuoteInPath() {
        let path = "/Volumes/it's mine/befold.app"
        let script = UpdateInstaller.updaterScript(
            appInDMG: path,
            installedApp: path,
            mountPoint: path,
        ...
        #expect(script.contains(#"'/Volumes/it'\''s mine/befold.app'"#))
    }
}
