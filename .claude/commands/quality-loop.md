...
- レビュー・自己チェックの出力に指摘が1件でも記載されていれば、たとえ
  「対応必須ではない」「任意」等の留保付きであっても**指摘ありとして扱う**。
  留保表現を理由に指摘ゼロと判定してはならない
- コーディネーター(メインエージェント)が指摘の要否を自己判断して Fix ステップ
  から除外することは禁止する。指摘リストは常に全件を Fix エージェントへ渡す
  (quality-loop.md 既存規定の「タグ問わず全件」を、コーディネーター自身の
  恣意的な除外からも守る)
- 指摘を却下してよいのは、Fix/Re-review エージェントが実際にコードを
  書き換えて比較した上で「現状維持が優れている」根拠を示した場合のみ。
  試さずに「妥当」と判断した却下は無効とし、次のラウンドで指摘として復活させる
