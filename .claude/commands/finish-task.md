...
- 構成図・モジュールツリー・コンポーネント一覧に出てくる型を追加・削除・改名した
- 型の責務や、型どうしの依存の向きを変えた
- 永続化キー・表示モード・ウィンドウ状態など、文書に列挙されている値の集合を変えた
- FeatureGate を新設・撤去した
...
（「native-app-design.md への反映は不要（内部実装のみで公開構成に変化なし）」）。
黙って飛ばすと、次に読む人には「反映漏れ」と区別が付かない。

不可逆な設計判断（他の選択肢を潰す判断）を下したなら、あわせて
`backlog decision create` で ADR に残す。
