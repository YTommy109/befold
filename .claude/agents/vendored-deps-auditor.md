...
`mermaid.min.js`・`markdown-it.min.js`・`highlight.min.js`・`dompurify.min.js`・`github-markdown.css`(+ hljs テーマ CSS)は `BefoldApp/BefoldKit/Resources/` に
...
1. 同梱バージョンを特定する:
   - `head -1 BefoldApp/befold/Resources/markdown-it.min.js`（先頭バナー）
   - `head -1 BefoldApp/befold/Resources/github-markdown.css`（先頭バナー）
   - `grep -o 'version[":]*[0-9.]*' BefoldApp/befold/Resources/mermaid.min.js | head`
     等でミニファイド JS 内の版文字列を探す。
   - `grep -o 'versionString="[0-9.]*"' BefoldApp/befold/Resources/highlight.min.js | head -1`
   - `head -1 BefoldApp/BefoldKit/Resources/dompurify.min.js`（先頭バナーに版番号あり）
   - `BefoldApp/package.json` の記録と一致するか確認する。
2. WebSearch / WebFetch で調査する:
   ...
   - 該当する脆弱性が、アプリの実際の設定（例: markdown-it の `html:true`、
     `linkify` / `typographer` の有効化、mermaid の `securityLevel`、
     DOMPurify を設定なしのデフォルトで呼んでいること）で
     **実際に発火するか**を `viewer.html` の初期化コードと突き合わせて評価する。
   - DOMPurify は CSP が `script-src 'unsafe-inline'` を許可しているため唯一の XSS 防御である点を踏まえ、
     サニタイズバイパス系 CVE を最優先で確認する。
3. `BefoldApp/package.json` に `node_modules` があれば `npm audit` も実行する。
