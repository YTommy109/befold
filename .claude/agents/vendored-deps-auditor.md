...
description: 手動ベンダリングされた同梱 JS ライブラリ（mermaid / markdown-it）のバージョンずれと既知脆弱性を監査する。リリース前や依存を気にするときに使う。
...
`mermaid.min.js` と `markdown-it.min.js` は `MmdviewApp/mmdview/Resources/` に
...
2. WebSearch / WebFetch で調査する:
   ...
   - 同梱版に該当する既知脆弱性（CVE / GitHub Security Advisory）。特に
     mermaid の XSS、markdown-it の ReDoS / DoS / XSS。
