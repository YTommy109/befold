...
macOS ネイティブのファイルビューアです。ライブリロード・あいまいファイル検索・git 差分の並列表示を備えています。
...
プレビューの手段は、たいてい何かを諦めることになります。そのエディタでしか動かないプラグインか、
手で更新するブラウザのタブか、手元のファイルを貼り付ける Web サービスか、のどれかです。

befold は、ただファイルを開くだけの単体の macOS アプリです。プロセス内でレンダリングし、
ファイルを監視して、保存した瞬間に描き直します。エディタもサーバーもアップロードも要りません。

## 安全に読める — ローカル完結

描画はすべて手元で行い、開いた文書について何かが外へ出ることはありません。

- **文書を送信しない。** befold が出す通信はアップデート確認（`appcast.xml`）だけで、
  ファイルの中身も名前もパスも含みません。
- **文書の側からも外へ出させない。** レンダリング中のページからの `http(s)://` と
  `ws(s)://` のリクエストを `WKContentRuleList` で遮断するため、文書に埋め込まれた
  リモート画像・トラッキング用の画像・Web フォント・外部スクリプトは読み込まれません。
- **開いた HTML のスクリプトは実行しない。** HTML の直接表示では JavaScript を無効にし、
  Markdown は `script-src 'self'` のもとで DOMPurify を通します。
- **ファイルを書き換えない。** 開いたファイルへの書き込み経路はありません。git 連携は
  リポジトリを読むだけで、フックもフィルタも実行しません。

配布物は Developer ID で署名し、notarize と staple 済みで、アップデートは Ed25519 の署名で
検証します。なお befold 本体はサンドボックスではありません（サンドボックスなのは QuickLook
拡張のほうです）。

## こんなときに使う

**AI コーディングエージェントの出力をその場で確認する。**
エージェントは Mermaid の図や Markdown の仕様書を次々に吐き出します。`befold docs/design.mmd` で
レンダリング済みの状態で開いておけば、エージェントが書き換えるたびにウィンドウが自動で描き直されます。
mermaid.live に貼り直す必要はありません。

**ターミナルで完結する環境に、プレビューペインを足す。**
Claude Code や Codex、vim、素のシェルで作業していると、そもそも開けるプレビューペインがありません。
befold がそのペインになります。`befold spec.md` を一度実行してウィンドウをターミナルの隣に置けば、
保存のたびに描き直されます。読むだけなので、他のツールとファイルを取り合いません。

**知らないリポジトリを読み解く。**
`⌘P` でツリー内のどのファイルにもあいまい検索で飛べます。`⌘+クリック` でリンクや参照先ファイルを開き、
`⌘[` / `⌘]` で履歴を行き来できます。IDE に期待する移動手段が、設定不要のビューアにあります。

**変更を差分で読む。**
git リポジトリ内であれば、ソース表示に差分を並べて表示し、変更されたファイルをサイドバーに印で示します。

**Finder から、何も開かずにプレビューする。**
QuickLook 拡張を同梱しているので、`.mmd` や `.md` にスペースキーを押すだけでレンダリング結果が出ます。
...
- **対応フォーマット**: Mermaid (.mmd / .mermaid) / Markdown (.md / .markdown) / SVG / HTML / CSV / TSV をレンダリング表示し、PNG / JPG / GIF / WebP / BMP / ICO / PDF を表示します。30 以上の言語（49 拡張子）はシンタックスハイライト付きでソース表示します
- **レンダリング / ソース切替**: ⌘U でレンダリング結果とソース表示を切り替えられます（⌘L で行番号を表示切替）
- **ライブリロード**: ファイルを保存すると自動でプレビューが更新されます（0.2s デバウンス）
- **大きなファイルの段階描画**: Markdown / CSV・TSV / ソースコードは先頭から順に読み込んで描画するため、巨大なファイルでも待たされません
- **ズーム**: ⌘+ / ⌘- / ⌘0 で拡大・縮小・等倍に戻せます
- **ページ内検索**: ⌘F で検索し、⌘G / ⇧⌘G で前後の一致へ移動できます
- **Finder クイックルック**: QuickLook 拡張を同梱しているので、Finder でスペースキーを押すだけでアプリを開かずにプレビューできます

### ファイルの行き来

- **サイドバー**: ⌘S で開閉します。同じフォルダーのファイルを一覧し、フォルダー優先／アルファベット順の並び替え、名前での絞り込み、⌃⌘H での不可視ファイルの表示切替ができます。git リポジトリ内なら、相対パスの基準をリポジトリルートに合わせます
- **Quick Open**: ⌘P でパスをあいまい検索し、キーボードだけでファイルを開けます
- **ファイル参照ジャンプ**: ⌘+クリックでリンクや参照先ファイルを開けます。⌘[ / ⌘]（または二本指スワイプ）で戻る・進むができます
- **ブックマーク**: ⌘D でよく開くファイルに印を付け、ブックマークメニューから開き直せます
- **最近使った項目 / リポジトリ**: 直近のファイルに加え、リポジトリ単位でも前回のタブ構成ごと開き直せます

### ウィンドウとアプリ

- **タブ & セッション復元**: macOS ネイティブタブに対応し、前回のウィンドウ・タブ構成を自動で復元します
- **設定**: ソース表示に使うコードフォントのファミリーとサイズを変更できます（⌘,）
- **アプリ内アップデート**: 新バージョンを通知し、ワンクリックで更新できます
- **ヘルプ**: 機能説明・キーボードショートカット一覧・AI コーディングエージェント連携の手引きをヘルプメニューから参照できます
